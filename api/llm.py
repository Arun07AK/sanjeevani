...
_PLANNER_SYSTEM = (
    ...
    "Core doctrine: the RAIL must FIX the CAUSE; the CHANNEL must RESPECT the contact "
    "constraints (feature phone = no app/V-CIP/WhatsApp; no_whatsapp = use voice/SMS). "
    "Language is not a constraint — every message is in the account's language.\n\n"
    "Ideal-play doctrine (cause -> ideal channel / cause-correct rail):\n"
    "- stale_kyc + smartphone: whatsapp or ivr_voice / vcip (best); yono_inb no-change "
    "self-update or bc_visit are workable. NEVER atm — an ATM transaction does NOT "
    "refresh KYC, so it does not fix stale_kyc.\n"
    "- stale_kyc + feature phone: ivr_voice or sms / bc_visit is the ONLY cause-correct "
    "rail (vcip and yono_inb need a smartphone). NEVER atm.\n"
    "- never_first_txn: whatsapp / ivr_voice / sms / atm (best) or yono_inb (smartphone); "
    "the message explains the ONE first customer-led transaction that activates the account.\n"
    "- disengaged: whatsapp / ivr_voice / sms / yono_inb (smartphone) or atm; the message "
    "is a warm nudge naming what restarts activity (and the DBT benefit when interrupted).\n"
    ...
    "is one concrete sentence naming the CAUSE and why the rail fixes it (it may cite a "
    "contact constraint as the reason for the channel — never as the cause)."
)
